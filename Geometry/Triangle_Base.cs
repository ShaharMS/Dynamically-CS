...
public partial class Triangle : DraggableGraphic
{
    ...
    {
        Circumcircle = Tools.CircleFrom3Vertices(Vertex1, Vertex2, Vertex3);
    ...
    public List<(TriangleType type, string details, double confidence)> SuggestTypes()
    {
        ...
        {
            if (Type != TriangleType.ISOSCELES && Type != TriangleType.ISOSCELES_RIGHT && Segment12.Length.IsSimilarTo(Segment13.Length, Settings.MakeIsoscelesSideRatioDiff)) l.Add((TriangleType.ISOSCELES, $"{Segment12} = {Segment13}", Segment12.Length.GetSimilarityPercentage(Segment13.Length)));
            if (Type != TriangleType.ISOSCELES && Type != TriangleType.ISOSCELES_RIGHT && Segment12.Length.IsSimilarTo(Segment23.Length, Settings.MakeIsoscelesSideRatioDiff)) l.Add((TriangleType.ISOSCELES, $"{Segment12} = {Segment23}", Segment12.Length.GetSimilarityPercentage(Segment23.Length)));
            if (Type != TriangleType.ISOSCELES && Type != TriangleType.ISOSCELES_RIGHT && Segment23.Length.IsSimilarTo(Segment13.Length, Settings.MakeIsoscelesSideRatioDiff)) l.Add((TriangleType.ISOSCELES, $"{Segment23} = {Segment13}", Segment23.Length.GetSimilarityPercentage(Segment13.Length)));
        }
    ...
    {
        var c = Vertex.GetVertexById(cid);
        if (c == null) return false;
        var j1 = Vertex.GetVertexById(id1);
        if (j1 == null) return false;
        var j2 = Vertex.GetVertexById(id2);
