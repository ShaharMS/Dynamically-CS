        ...
        rotate.Click += (sender, e) =>
        {
            ...
            void Move(object? sender, PointerEventArgs args)
            {
                ...
                if (Subject.Vertex1.IsMovementLegal(rotationCenter.X + dist1 * Math.Cos(rad1 + currentRotation), rotationCenter.Y + dist1 * Math.Sin(rad1 + currentRotation)) &&
                    ...
                    Subject.Vertex2.IsMovementLegal(rotationCenter.X + dist3 * Math.Cos(rad3 + currentRotation), rotationCenter.Y + dist3 * Math.Sin(rad3 + currentRotation)) &&
                    Subject.Vertex2.IsMovementLegal(rotationCenter.X + dist4 * Math.Cos(rad4 + currentRotation), rotationCenter.Y + dist4 * Math.Sin(rad4 + currentRotation))) return;
